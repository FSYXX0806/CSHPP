...
class ViT(nn.Module):
    ...
    def __init__(
        ...
        seq_len = gh * gw  # 将图片的patch块分为多个位置
...
#         exit(0)
        ...
        # Classifier head 分类器的输入头 首先归一化之后一个线性层将输入映射到类别向量空间
        ...
        if pretrained:
            ...
            # PRETRAINED_MODELS[name]['num_classes'] #edit for new
...
#             exit(0)
            ...
            print('load_first_conv:', in_channels == pretrained_num_channels,
                  'load_fc:', num_classes == pretrained_num_classes)
#         self.idx=[0]*1000
#         self.dict_={}
    ...
    def forward(self, x):
        ...
        # print(f'before patch :{x.size()}') #before patch :torch.Size([64, 3, 384, 384])
        x = self.patch_embedding(x)  # b,d,gh,gw
        # print(f'patch embedding:{x.size()}') #patch embedding:torch.Size([64, 768, 12, 12])
        x = x.flatten(2).transpose(1, 2)  # b,gh*gw,d    y
        # print(f'flatten :{x.size()}') #flatten :torch.Size([64, 144, 768])
